...
s32 intro_play_its_a_me_mario(void) {
    ...
    aiModeSeed[0] = rng_u8() % 0x30;
    save_file_set_ai_seed();
    if (aiModeSeed[0] == 26) {
        randSelect = rng_u8() % 0x3;
        if (randSelect == 1) {
            seriousMode = 1;
            return 2;
        } else {
...
void print_error(void) {
    errorTextAlpha = 50 * sinf(gGlobalTimer/4) + 200;
    print_string_text_colored("An error has occured in the game loop.", 20, 210, 254, 254, 254, 255);
    print_string_text_colored("Please give us a call at 1-800-255-3700", 20, 194, 254, 254, 254, 255);
    print_string_text_colored("and report the error. Thank you!", 20, 178, 254, 254, 254, 255);
    print_string_text_colored("0x010B12F0 ...", 20, 66, 254, 0, 0, errorTextAlpha);
    print_string_text_colored("Failed to load level data", 20, 50, 254, 0, 0, errorTextAlpha);
}
