...
float decay;
float bounceTimer = 20;

void bhv_heart_loop(void) {
    bounceTimer++;
    decay += 1.5f;
    o->header.gfx.scale[0] = (-0.04f * cosf(bounceTimer/decay) + 1.0f);
    o->header.gfx.scale[1] = (0.04f * sinf(bounceTimer/decay) + 1.0f);
    o->header.gfx.scale[2] = (0.04f * sinf(bounceTimer/decay) + 1.0f);
    cur_obj_play_sound_2(SOUND_AIR_HEART);
    cur_obj_play_sound_2(SOUND_ENV_CHOIR);

    if (decay >= 50.0f) {
        bounceTimer = 20;
        decay = 0.0f;
        cur_obj_play_sound_2(SOUND_OBJ_HEART_BEAT);
    }

}

void bhv_heart_clockhand_loop(void) {
