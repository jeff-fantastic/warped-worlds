...
void koopakid_act_idle(void) {
    s32 type;
    struct Object *sp24;
    sp24 = cur_obj_find_nearby_held_actor(bhvStackOfBooks, 500);
    ...
    if (type == 1) {
        ...
        if (sp24 != NULL && o->oKoopaKidBookReturned == 0 && sp24->oHeldState != HELD_FREE) {
            o->oAction = 4;
        }
    } else {
        cur_obj_init_animation(1);
...
void koopakid_act_star(void) {
    switch (o->oSubAction) {
        case 0:
            if (!cur_obj_is_mario_on_platform()) {
                if (cur_obj_update_dialog_with_cutscene(1, 1, CUTSCENE_DIALOG, DIALOG_173)) {
                    o->oSubAction = 1;
                }
            }
            break;
        case 1:
            cur_obj_spawn_star_at_y_offset(o->oHomeX - 400, o->oHomeY + 350, o->oHomeZ, 200.0f);
            o->oKoopaKidBookReturned = 1;
            o->oAction = BOBOMB_BUDDY_ACT_IDLE;
            break;
    }
}

void koopakid_act_turn_to_talk(void) {
    o->oMoveAngleYaw = approach_s16_symmetric(o->oMoveAngleYaw, o->oAngleToMario, 0x240);
    if ((s16) o->oMoveAngleYaw == (s16) o->oAngleToMario) {
        o->oAction = BOBOMB_BUDDY_ACT_TALK;
    }
...
void koopakid_act_talk(void) {
    if (set_mario_npc_dialog(1) == 2) {
        o->activeFlags |= ACTIVE_FLAG_INITIATED_TIME_STOP;

        switch (o->oBobombBuddyRole) {
            case BOBOMB_BUDDY_ROLE_ADVICE:
                if (o->oKoopaKidBookReturned == 0) {
                    ...
                    if (cutscene_object_with_dialog(CUTSCENE_DIALOG, o, DIALOG_174)!= BOBOMB_BUDDY_BP_STYPE_GENERIC) {
                        set_mario_npc_dialog(0);
