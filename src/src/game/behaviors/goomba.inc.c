 ...
 * Hitbox for goomba.
 */
static struct ObjectHitbox sRedGoombaHitbox = {
    /* interactType:      */ INTERACT_UNKNOWN_08,
    /* downOffset:        */ 0,
    /* damageOrCoinValue: */ 2,
    /* health:            */ 0,
    /* numLootCoins:      */ 3,
 ...
 * Properties for regular, huge, and tiny goombas.
 */
static struct RedGoombaProperties sRedGoombaProperties[] = {
    { 1.5f, SOUND_OBJ_ENEMY_DEATH_LOW, 4000, 2 },
    { 3.5f, SOUND_OBJ_ENEMY_DEATH_LOW, 4000, 2 },
    { 0.5f, SOUND_OBJ_ENEMY_DEATH_HIGH, 1500, 2 },
};

/**
 * Attack handlers for goombas.
 */
static u8 sRedGoombaAttackHandlers[][6] = {
    // regular and tiny
    {
        /* ATTACK_PUNCH:                 */ ATTACK_HANDLER_KNOCKBACK,
        /* ATTACK_KICK_OR_TRIP:          */ ATTACK_HANDLER_KNOCKBACK,
 ...
 * Initialization function for goomba.
 */
void bhv_red_goomba_init(void) {
    o->oGoombaSize = o->oBehParams2ndByte & GOOMBA_BP_SIZE_MASK;

    o->oGoombaScale = sRedGoombaProperties[o->oGoombaSize].scale;
    o->oDeathSound = sRedGoombaProperties[o->oGoombaSize].deathSound;

    obj_set_hitbox(o, &sRedGoombaHitbox);

    o->oDrawingDistance = sRedGoombaProperties[o->oGoombaSize].drawDistance;
    o->oDamageOrCoinValue = sRedGoombaProperties[o->oGoombaSize].damage;
 ...
 * Enter the jump action and set initial y velocity.
 */
static void red_goomba_begin_jump(void) {
    cur_obj_play_sound_2(SOUND_OBJ_GOOMBA_ALERT);
    o->oAction = GOOMBA_ACT_JUMP;
    o->oForwardVel = 0.0f;
    o->oVelY = 50.0f / 3.0f * o->oGoombaScale;
 ...
 * comes back.
 */
static void mark_red_goomba_as_dead(void) {
    if (o->parentObj != o) {
        set_object_respawn_info_bits(o->parentObj,
                                     (o->oBehParams2ndByte & GOOMBA_BP_TRIPLET_FLAG_MASK) >> 2);
 ...
 * chase him.
 */
static void red_goomba_act_walk(void) {
    treat_far_home_as_mario(1000.0f);

    obj_forward_vel_approach(o->oGoombaRelativeSpeed * 1.2f, 2.2f);

    // If walking fast enough, play footstep sounds
    if (o->oGoombaRelativeSpeed > 5.0f / 3.0f) {
        cur_obj_play_sound_at_anim_range(2, 17, SOUND_OBJ_GOOMBA_WALK);
    }
    ...
    if (o->oGoombaTurningAwayFromWall) {
        ...
        cur_obj_rotate_yaw_toward(o->oGoombaTargetYaw, 0x500);
    }
}
 ...
 * attacks a huge goomba with an attack that doesn't kill it.
 */
static void red_goomba_act_attacked_mario(void) {
    if (o->oGoombaSize == GOOMBA_SIZE_TINY) {
        mark_red_goomba_as_dead();
        o->oNumLootCoins = 0;
        obj_die_if_health_non_positive();
    } else {
        //! This can happen even when the goomba is already in the air. It's
 ...
 * Move until landing, and rotate toward target yaw.
 */
static void red_goomba_act_jump(void) {
    obj_resolve_object_collisions(NULL);
 ...
 * Update function for goomba.
 */
void bhv_red_goomba_update(void) {
    // PARTIAL_UPDATE
    ...
    if (obj_update_standard_actions(o->oGoombaScale)) {
        ...
        if ((animSpeed = o->oForwardVel / o->oGoombaScale * 0.4f) < 1.2f) {
            animSpeed = 1.2f;
        }
        cur_obj_init_animation_with_accel_and_sound(0, animSpeed);
        ...
        // and will not respawn if Mario leaves and re-enters the spawner's radius
        // even though the goomba isn't actually dead.
        if (obj_handle_attacks(&sRedGoombaHitbox, GOOMBA_ACT_ATTACKED_MARIO,
                               sGoombaAttackHandlers[o->oGoombaSize & 1])) {
            mark_red_goomba_as_dead();
        }
