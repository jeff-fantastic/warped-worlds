...
void bookstack_get_dropped(void) {
    cur_obj_become_tangible();
    cur_obj_enable_rendering();
    cur_obj_get_dropped();
    o->header.gfx.node.flags &= ~GRAPH_RENDER_INVISIBLE;
...
void bookstack_get_thrown(void) {
    cur_obj_become_tangible();
    cur_obj_enable_rendering_2();
    cur_obj_enable_rendering();
    o->header.gfx.node.flags &= ~GRAPH_RENDER_INVISIBLE;
...
void bookstack_released_loop(void) {
    o->oBreakableBoxSmallFramesSinceReleased++;
    ...
    // Despawn, and create a corkbox respawner
    if (o->oBreakableBoxSmallFramesSinceReleased > 900) {
        create_respawner(MODEL_BOB_BOOKSTACK, bhvStackOfBooks, 3000);
        o->activeFlags = ACTIVE_FLAG_DEACTIVATED;
    }
}

void bookstack_idle_loop(void) {
    switch (o->oAction) {
        case 0:
            bookstack_act_move();
            break;
    ...
    if (o->oBreakableBoxSmallReleased == 1)
        bookstack_released_loop();
}

void bhv_bookstack_loop(void) {
    switch (o->oHeldState) {
        case 0:
            bookstack_idle_loop();
            break;
        ...
        case 2:
            bookstack_get_thrown();
            break;

        case 3:
            bookstack_get_dropped();
            break;
    }
