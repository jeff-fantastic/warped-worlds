...
s32 whomp_king_spawn_shockwave(void) {
    struct Object *wave;
    wave = spawn_object(o, MODEL_WHOMP_WAVE, bhvWhompShockWave);
    wave->oPosY = o->oFloorHeight;
    wave->oFaceAnglePitch = 0;
    if (wave->oTimer == 1) {
        wave->oFaceAngleYaw = o->oFaceAngleYaw;
    }
    return 1;
}

void whomp_act_0(void) { // Whomp init
    
    type = (o->oBehParams >> 24) & 0xFF;

    cur_obj_init_animation_with_accel_and_sound(0, 1.0f);
    cur_obj_set_pos_to_home();
    if (o->oBehParams2ndByte != 0) { // branch off into king whomp behavior
        gSecondCameraFocus = o;
        cur_obj_scale(2.0f);
        if (o->oSubAction == 0) {
            if (o->oDistanceToMario < 600.0f) { // look for mario in radius
                o->oSubAction++; // prompt dialogue
                seq_player_lower_volume(SEQ_PLAYER_LEVEL, 60, 40);
            } else {
                cur_obj_set_pos_to_home(); // reset pos and health
                o->oHealth = 3;
            }
        } else if (o->oSubAction == 1)
            if (type != 1) { // branch off from normal and rematch fight
                if (cur_obj_update_dialog_with_cutscene(2, 1, CUTSCENE_DIALOG, DIALOG_114)) {
                    o->oAction = 2;
                }
            } else {
                if (cur_obj_update_dialog_with_cutscene(2, 1, CUTSCENE_DIALOG, DIALOG_114)) {
...
void whomp_act_2(void) { // whomp walk towards mario
    s16 sp1E;
    if (type != 1) {
        cur_obj_init_animation_with_accel_and_sound(0, 1.0f);
        o->oForwardVel = 3.0f;
    ...
    if (o->oTimer > 30) {
        ...
        if (sp1E < 0x2000) {
            if (o->oDistanceToMario < 1500.0f) { // if mario is close, speed up
                if (type != 1) {
                    o->oForwardVel = 9.0f;
                    cur_obj_init_animation_with_accel_and_sound(0, 3.0f);
                }
            } 
            if (o->oDistanceToMario < 300.0f && type == 0) { // if mario is closer, fall on mario
                o->oAction = 3;
            } else if (o->oDistanceToMario < 600.0f && type == 1) {
                o->oAction = 3;
            }
        }
    }
    whomp_play_sfx_from_pound_animation();
    if (mario_is_far_below_object(1000.0f)) { // if mario is away from boss, disengage
        o->oAction = 0;
        stop_background_music(SEQUENCE_ARGS(4, SEQ_EVENT_BOSS));
    }
}

void whomp_act_3(void) { // whomp prepares for jump up
    if (type != 1) {
        o->oForwardVel = 0.0f;
...
void whomp_act_4(void) { // whomp jumps up and falls
    if (o->oTimer == 0) {
        if (type != 1) {
            o->oVelY = 40.0f;
        } else {
        ...
        if (o->oFaceAnglePitch > 0x4000) {
            ...
            if (mario_is_far_below_object(20.0f)) {
                o->oAction = 5;
            }
        }
    }
...
void whomp_act_5(void) {
    if (o->oSubAction == 0 && o->oMoveFlags & OBJ_MOVE_LANDED) { // falling
        if (type != 1) {
...
void whomp_act_8(void) { // whomp king defeat
    if (o->oBehParams2ndByte != 0) {
        if (cur_obj_update_dialog_with_cutscene(2, 2, CUTSCENE_DIALOG, DIALOG_115)) {
            ...
            cur_obj_shake_screen(SHAKE_POS_SMALL);
            o->oPosY += 100.0f;
            spawn_default_star(0.0f, 377.0f, -1162.0f);
            cur_obj_play_sound_2(SOUND_OBJ_KING_WHOMP_DEATH);
            o->oAction = 9;
        }
    } else {
...
void whomp_act_9(void) { // stop boss music
    if (o->oTimer == 60)
        stop_background_music(SEQUENCE_ARGS(4, SEQ_EVENT_BOSS));
}
