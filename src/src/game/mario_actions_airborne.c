...
void play_knockback_sound(struct MarioState *m) {
    if (m->actionArg == 0 && (m->forwardVel <= -28.0f || m->forwardVel >= 28.0f)) {
        if (Character == 1) {
            play_sound_if_no_flag(m, SOUND_LUIGI_DOH, MARIO_MARIO_SOUND_PLAYED);
        } else {
            play_sound_if_no_flag(m, SOUND_MARIO_DOH, MARIO_MARIO_SOUND_PLAYED);
        }
    } else {
        if (Character == 1) {
            play_sound_if_no_flag(m, SOUND_LUIGI_UH, MARIO_MARIO_SOUND_PLAYED);
        } else {
    ...
    if (m->action != ACT_TWIRLING && m->floor->type != SURFACE_BURNING) {
        if (m->vel[1] < -55.0f) {
            if (fallHeight > 3000.0f) {
                ...
                if (Character == 1) {
                    play_sound(SOUND_LUIGI_ATTACKED, m->marioObj->header.gfx.cameraToObject);
                } else {
...
#ifdef VERSION_JP
        if (Character == 1) {
            play_sound(SOUND_LUIGI_OOOF, m->marioObj->header.gfx.cameraToObject);
        } else {
            play_sound(SOUND_MARIO_OOOF, m->marioObj->header.gfx.cameraToObject);
        }
#else
        if (Character == 1) {
            play_sound(SOUND_LUIGI_OOOF2, m->marioObj->header.gfx.cameraToObject);
        } else {
...
s32 act_double_jump(struct MarioState *m) {
    ...
    if (Character == 1) {
        play_mario_sound(m, SOUND_ACTION_TERRAIN_JUMP, SOUND_LUIGI_HOOHOO);
    } else {
...
s32 act_backflip(struct MarioState *m) {
    ...
    if (Character == 1) {
        play_mario_sound(m, SOUND_ACTION_TERRAIN_JUMP, SOUND_LUIGI_YAH_WAH_HOO);
    } else {
    ...
    if (Character == 1) {
        if (m->vel[1] <= 10.0f) {
            play_sound(SOUND_LUIGI_PUNCH_HOO, m->marioObj->header.gfx.cameraToObject);
            ...
            if (m->pos[1] + yOffset + 160.0f < m->ceilHeight) {
                ...
                if (Character == 1) {
                    if (m->input & INPUT_B_PRESSED) {
                        ...
                        set_mario_y_vel_based_on_fspeed(m, 15.0f, 0.5f);
                        m->particleFlags |= PARTICLE_MIST_CIRCLE;
...
s32 act_shot_from_cannon(struct MarioState *m) {
    ...
    if (Character == 1) {
        play_sound_if_no_flag(m, SOUND_LUIGI_YAHOO, MARIO_MARIO_SOUND_PLAYED);
    } else {
...
#ifndef VERSION_JP
        if (Character == 1) {
            play_sound(SOUND_LUIGI_YAHOO_WAHA_YIPPEE + ((gAudioRandom % 5) << 16), m->marioObj->header.gfx.cameraToObject);
        } else {
