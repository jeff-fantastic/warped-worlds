...
#endif
            if (Character == 1) {
                play_sound(SOUND_LUIGI_YAH_WAH_HOO + ((gAudioRandom % 3) << 16), m->marioObj->header.gfx.cameraToObject);
            } else {
...
static u32 set_mario_action_airborne(struct MarioState *m, u32 action, u32 actionArg) {
    ...
    switch (action) {
        ...
        case ACT_TRIPLE_JUMP:
            ...
            m->forwardVel *= 0.8f;
            if (Character == 1) {
                set_mario_action(m, ACT_SPECIAL_TRIPLE_JUMP, 1);
            }
            break;
...
s32 transition_submerged_to_airborne(struct MarioState *m) {
    set_camera_mode(m->area->camera, m->area->camera->defMode, 1);
    gMarioState->faceAngle[1] = currentAngle - 0x4000;
...
#ifdef ShowMarioPos
        print_text_fmt_int(210, 136, "X %d", (int)m->pos[0]);
        print_text_fmt_int(210, 120, "Y %d", (int)m->pos[1]);
        print_text_fmt_int(210, 104, "Z %d", (int)m->pos[2]);
#endif
...
void init_mario_from_save_file(void) {
    ...
    gMarioState->numKeys = save_file_get_key_count(gCurrSaveFileNum - 1);
