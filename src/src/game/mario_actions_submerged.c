...
static s32 act_water_shocked(struct MarioState *m) {
    if (Character == 1) {
        play_sound_if_no_flag(m, SOUND_LUIGI_WAAAOOOW, MARIO_ACTION_SOUND_PLAYED);
    } else {
...
static s32 act_drowning(struct MarioState *m) {
    ...
    if (Character == 1) {
        play_sound_if_no_flag(m, SOUND_LUIGI_DROWNING, MARIO_ACTION_SOUND_PLAYED);
    } else {
...
static s32 act_water_plunge(struct MarioState *m) {
    ...
    if (m->actionState == 0) {
        ...
        if (m->peakHeight - m->pos[1] > 1150.0f) {
            if (Character == 1) {
                play_sound(SOUND_LUIGI_HAHA_2, m->marioObj->header.gfx.cameraToObject);
            } else {
