    ...
    switch (gCurrLevelNum) {
        ...
        default:
            if (!(save_file_get_star_flags(fileIndex, courseIndex) & starFlag)) {
                ...
                gSaveBuffer.files[gCurrSaveFileNum - 1][0].keyCount = gMarioState->numKeys;
            }
            break;
    }
}
...
s32 save_file_get_key_count(s32 fileIndex) {
    return gSaveBuffer.files[fileIndex][0].keyCount;
}
...
void save_file_set_ai_seed(void) {
    gSaveBuffer.menuData[0].seed = s_seed_a;
    gMainMenuDataModified = TRUE;
    save_main_menu_data();
}

u16 save_file_return_ai_seed1(void) {
    return gSaveBuffer.menuData[0].seed;
}
