...
s32 mario_update_punch_sequence(struct MarioState *m) {
    ...
    switch (m->actionArg) {
        case 0:
            if (Character == 1) {
                play_sound(SOUND_LUIGI_PUNCH_YAH, m->marioObj->header.gfx.cameraToObject);
            } else {
        ...
        case 3:
            if (Character == 1) {
                play_sound(SOUND_LUIGI_PUNCH_WAH, m->marioObj->header.gfx.cameraToObject);
            } else {
        ...
        case 6:
            if (Character == 1) {
                play_mario_action_sound(m, SOUND_LUIGI_PUNCH_HOO, 1);
            } else {
...
s32 act_picking_up(struct MarioState *m) {
    ...
    if (m->actionState == 0 && is_anim_at_end(m)) {
        ...
        if (Character == 1) {
            play_sound_if_no_flag(m, SOUND_LUIGI_HRMM, MARIO_MARIO_SOUND_PLAYED);
        } else {
    ...
    if (m->actionState == 1) {
        if (m->heldObj->oInteractionSubtype & (INT_SUBTYPE_GRABS_MARIO | INT_SUBTYPE_HEAVY)) {
...
s32 act_throwing(struct MarioState *m) {
    ...
    if (++m->actionTimer == 7) {
        ...
        if (Character == 1) {
            play_sound_if_no_flag(m, SOUND_LUIGI_WAH2, MARIO_MARIO_SOUND_PLAYED);
        } else {
...
s32 act_holding_bowser(struct MarioState *m) {
    ...
    if (m->input & INPUT_B_PRESSED) {
        ...
        if (m->angleVel[1] <= -0xE00 || m->angleVel[1] >= 0xE00) {
            if (Character == 1) {
                play_sound(SOUND_LUIGI_SO_LONGA_BOWSER, m->marioObj->header.gfx.cameraToObject);
            } else {
