...
static void set_mario_y_vel_based_on_fspeed(struct MarioState *m, f32 initialVelY, f32 multiplier) {
    ...
    m->vel[1] = 1.1f * initialVelY + get_additive_y_vel_for_jumps() + m->forwardVel * multiplier;
    }
...
s32 act_start_sleeping(struct MarioState *m) {
    ...
    if (m->actionState == 2 && animFrame == -1) {
        if (Character == 1) {
            play_sound(SOUND_LUIGI_YAWNING, m->marioObj->header.gfx.cameraToObject);
        } else {
    ...
    if (m->actionState == 1 && animFrame == -1) {
        if (Character == 1) {
            play_sound(SOUND_LUIGI_IMA_TIRED, m->marioObj->header.gfx.cameraToObject);
        } else {
...
s32 act_sleeping(struct MarioState *m) {
    ...
    switch (m->actionState) {
        case 0:
            ...
            if (animFrame == 2) {
                if (Character == 1) {
                    play_sound(SOUND_LUIGI_SNORING1, m->marioObj->header.gfx.cameraToObject);
                } else {
            ...
            if (animFrame == 20) {
                if (Character == 1) {
                    play_sound(SOUND_LUIGI_SNORING2, m->marioObj->header.gfx.cameraToObject);
                } else {
...
#ifndef VERSION_JP
            if (Character == 1) {
                play_sound(SOUND_LUIGI_SNORING3, m->marioObj->header.gfx.cameraToObject);
            } else {
...
s32 act_waking_up(struct MarioState *m) {
    if (!m->actionTimer) {
        ...
        stop_sound(SOUND_LUIGI_SNORING1, m->marioObj->header.gfx.cameraToObject);
        stop_sound(SOUND_LUIGI_SNORING2, m->marioObj->header.gfx.cameraToObject);
        stop_sound(SOUND_LUIGI_SNORING3, m->marioObj->header.gfx.cameraToObject);
    ...
    switch (m->actionState) {
        case 0:
            ...
            if (animFrame == 49) {
                ...
                if (Character == 1) {
                    play_sound(SOUND_LUIGI_PANTING_COLD, m->marioObj->header.gfx.cameraToObject);
                } else {
...
s32 act_coughing(struct MarioState *m) {
    ...
    if (animFrame == 25 || animFrame == 35) {
        if (Character == 1) {
            play_sound(SOUND_LUIGI_COUGHING1, m->marioObj->header.gfx.cameraToObject);
        } else {
    ...
    if (animFrame == 50 || animFrame == 58) {
        if (Character == 1) {
            play_sound(SOUND_LUIGI_COUGHING2, m->marioObj->header.gfx.cameraToObject);
        } else {
    ...
    if (animFrame == 71 || animFrame == 80) {
        if (Character == 1) {
            play_sound(SOUND_LUIGI_COUGHING3, m->marioObj->header.gfx.cameraToObject);
        } else {
...
//            return set_jumping_action(m, ACT_TWIRLING, 1);
//        } else {
        return set_jumping_action(m, ACT_BACKFLIP, 1);
    }
...
s32 act_panting(struct MarioState *m) {
    ...
    if (set_mario_animation(m, MARIO_ANIM_WALK_PANTING) == 1) {
        if (Character == 1) {
            play_sound(SOUND_LUIGI_PANTING + ((gAudioRandom % 3U) << 0x10),
